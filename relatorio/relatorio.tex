...
\begin{document}
    ...
    Para a variável contínua, foi escolhida a aceleração dos carros, medidas em milhas por segundo ao quadrado ($mph^2$). Essa medida é obtida fazendo o carro sair do repouso e chegar até $60mph$ e cronometrando o tempo gasto.
    ...
    \begin{table}[htbp]
        ...
        \begin{tabular}{cc}
            \toprule
            Medidada de dispersão                   & Estimativa ($mph^2$)\\
            \midrule
    ...
    Sumarizamos os dados na \autoref{table:aceleracao}. Foi necessário separar os dados em classes, dado a quantidade elevada de valores únicos.

    \begin{table}[htbp]
        \centering
        \begin{tabular}{C{2.5cm}|C{2.5cm}C{2.5cm}|C{2.5cm}C{2.5cm}|C{2.5cm}}
            \toprule
            Aceleração & Frequência & Frequência acumulada & Frequência relativa & Frequência relativa acumulada & $Pm_i$ \\
            \midrule
